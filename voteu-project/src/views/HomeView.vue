...
onMounted(async () => {
  const provinces = await ProvinceService.getProvincesByElection(years[0])
  if (!provinces) return
  ...
  for (const province of Object.values(provinces)) {
    provinceVotes.value[province.id] = 0
  }

  for (const year of years) {
    const voteData = await ProvinceService.getTotalVotesPerProvince(year)
    if (!voteData) continue

    for (const province of voteData) {
      if (provinceVotes.value[province.provinceId] !== undefined) {
        provinceVotes.value[province.provinceId] += province.votes
      } else {
        provinceVotes.value[province.provinceId] = province.votes
      }
    }
